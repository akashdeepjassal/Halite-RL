...
class NeuralNet(object):
    ...
    def __init__(self, cached_model=None, seed=None):
        ...
        with self._graph.as_default():
            ...
            seventh_layer = tf.contrib.layers.fully_connected(sixth_layer, self.SEVENTH_LAYER_SIZE)
            eight_layer = tf.contrib.layers.fully_connected(seventh_layer, self.EIGHTH_LAYER_SIZE)
            ninth_layer = tf.contrib.layers.fully_connected(eight_layer, 1, activation_fn=None)

            # Group the planets back in frames.
            logits = tf.reshape(ninth_layer, [-1, PLANET_MAX_NUM])
